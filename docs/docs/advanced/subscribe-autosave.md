...
- 回调是**微任务异步**投递的——同一次操作内跨 store 的多份中间状态会被折叠为一次事件,回调里读到的永远是操作完成后的完整快照。若需要同步读,用 `layoutBus.getSnapshot()`。
- 回调内**不要写回 store**(如 `useScene.setState`):写入若改变了数据指纹,会再次触发事件形成自持循环。
