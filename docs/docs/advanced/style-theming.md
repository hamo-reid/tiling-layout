...
const myTheme: LayoutConfig = {
  ...
  sizing:  { headerH: 30, corner: 18, radius: 8, splitter: 12, splitterLine: 2 }, // 头部高 / 角标 / 圆角 / 命中带宽 / 线宽(px)
  interaction: { dockCenter: 0.3, minAreaW: 0.08 },                            // 行为参数(⚠ 只能全局，见下)
};
```

只提供想改的字段,其余用默认值(区域间隔 2px、内容内边距 8px、舞台留白 0px、头部 26px、角标 14px、圆角 6px、命中带宽 6px、线宽 2px)。
...
- `sizing.splitter` 是分界线的**命中带宽**(拖拽热区宽度)。它与视觉线宽**解耦**:调大更好抓取,但不会把线变粗。
- `sizing.splitterLine` 是分界线的**视觉线宽**(命中带中央的细线)。可取任意值、可大于命中带宽,细线始终居中不偏移。
...
停靠热区、槽占比吸附网格、最小区域、角点阈值、命中容差、拖拽吸附分格、历史上限这类「操作手感」参数,由几何层/状态机消费——它们是**模块单例纯逻辑**,没有 React 上下文,所以**只能全局一份**,不能按实例区分。
...
const myTheme: LayoutConfig = {
  interaction: {
    ...
    edgeArm: 0.008,                  // 角点手势方向判定阈值(比例)
    hitTolerance: 0.008,             // findEdgeAtPos 命中容差(比例)
    snapDivisions: 8,                // 拖分界线的吸附分格数(行程区间等分)
    historyMax: 100,                 // undo/redo 历史上限(条数)
  },
};
```
...
非法值(非有限数 / ≤0 / 越界 / 非整数)逐键忽略、不阻断其余键;多次调用按栈还原。`hitTolerance` 只影响几何查询 `findEdgeAtPos`,内置分界线命中靠 `sizing.splitter` 的 DOM 带宽。
...
.tl-stage-wrap {
  --tl-region-gap: 6px;
  --tl-outer-gap: 12px;
  --tl-splitter: 12px;      /* 命中带宽 */
  --tl-splitter-line: 2px;  /* 视觉线宽 */
  --tl-radius: 2px;
}
.tl-stage-wrap .tl-area-box { border-color: #3b82f6; }
```

可用的变量:`--tl-region-gap`、`--tl-pad-region`、`--tl-outer-gap`、`--tl-header-h`、`--tl-corner`、`--tl-radius`、`--tl-splitter`、`--tl-splitter-line`,与 `spacing`/`sizing` 配置项一一对应。实例 `theme` 只把**显式配置的键**内联到 `.tl-stage-wrap`,因此未配置的键直接写在 `.tl-stage-wrap` 或更外层即可生效;若同一键既由实例 `theme` 配置又被 CSS 写入,以 `theme` 为准(内联优先级最高)。配色类令牌(面板、边框、强调色等)在 `tokens.css` 里,明暗切换跟着 `colorMode` 或 `data-theme` 走。所有类名和变量都带 `tl-` 前缀,不会和宿主项目冲突。
